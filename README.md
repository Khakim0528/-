...
Пока в `.env` не заданы `SMTP_*`, письма не отправляются, а код просто печатается в консоль сервера — удобно для разработки. Чтобы письма реально уходили, впишите данные своего почтового сервиса:

- **Gmail:** включите двухэтапную аутентификацию, создайте [пароль приложения](https://myaccount.google.com/apppasswords), укажите `SMTP_HOST=smtp.gmail.com`, `SMTP_PORT=587`, `SMTP_USER=ваша_почта@gmail.com`, `SMTP_PASSWORD=пароль_приложения`.
- **Или** любой сервис для рассылок с бесплатным тарифом (Brevo, Mailgun и т.п.) — впишите его SMTP-данные так же.
