...
Администратор создаётся при старте из `ADMIN_EMAIL` / `ADMIN_PASSWORD` в `.env` (и всегда считается подтверждённым — код подтверждения ему не нужен).

## Подтверждение email при регистрации

После `POST /auth/register` на почту уходит 6-значный код (`POST /auth/verify-email` его проверяет и сразу выдаёт токен; `POST /auth/resend-code` отправляет новый). Войти без подтверждения нельзя. Код живёт 15 минут, попыток ввести — 5.

Пока не настроено ничего из списка ниже, письма не отправляются, а код просто печатается в консоль сервера — удобно для разработки.

- **На Render (или любом хостинге, блокирующем исходящий SMTP на бесплатном плане) — через Brevo (рекомендуется):** зарегистрируйтесь на [brevo.com](https://www.brevo.com) (бесплатно, 300 писем/день), подтвердите свой email как отправителя (Senders), возьмите API-ключ в **SMTP & API → API Keys**. Задайте `BREVO_API_KEY` и `SMTP_FROM` (тот самый подтверждённый адрес). Работает через обычный HTTPS-запрос, поэтому не упирается в блокировку SMTP-портов.
- **Для локальной разработки — через Gmail по SMTP:** включите двухэтапную аутентификацию, создайте [пароль приложения](https://myaccount.google.com/apppasswords), укажите `SMTP_HOST=smtp.gmail.com`, `SMTP_PORT=587`, `SMTP_USER=ваша_почта@gmail.com`, `SMTP_PASSWORD=пароль_приложения`. На бесплатном Render не заработает — сам Render с сентября 2025 блокирует исходящие SMTP-порты (25/465/587) на бесплатном плане.
